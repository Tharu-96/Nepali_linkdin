...
export default function Login() {
  ...
  const roleLabel = selectedRole === "employer" ? "employer" : "worker";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const signedInUser = await login(email, password, selectedRole);
      if (signedInUser?.role && signedInUser.role !== selectedRole) {
        setError(`This account is registered as ${signedInUser.role}. Please sign in as ${signedInUser.role}.`);
        return;
      }
      navigate("/dashboard");
    } catch (err) {
      setError(err.response?.data?.detail || err.message || `Login failed. Check your credentials and ${roleLabel} role.`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-card border border-slate-100">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-600">Sign in to your Rozgar account</p>
        </div>
        ...
        <div className="mt-8">
          <label className="block text-sm font-medium text-slate-700 mb-2">I am signing in as a</label>
          <select 
            value={selectedRole} 
